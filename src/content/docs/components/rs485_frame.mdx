...
| Device | UART | `crc.type` | `tx.gate.frame_type` | `command_format` |
|---|---|---|---|---|
| Hayward AquaLogic wireless remote | 19200 8N2 | `sum16_big_endian` | `[0x01, 0x01]` | preamble `[0x00, 0x83, 0x01]`, size 4, repeat 2, postamble `[0x00]` |
| Hayward AquaLogic wired remote | 19200 8N2 | `sum16_big_endian` | `[0x01, 0x01]` | preamble `[0x00, 0x03]`, size 4, repeat 2 |
| Hayward AquaLogic wired local / main panel | 19200 8N2 | `sum16_big_endian` | `[0x01, 0x01]` | preamble `[0x00, 0x02]`, size 4, repeat 2 |
| Jandy AquaLink RS AllButton | 9600 8N1 | `sum8`, `rx_accept: [header_inclusive]` | `[0x08, 0x00]` | preamble `[0x00, 0x01, 0x80]`, size 1 |
...
- **command_format** (*Optional*): How a 32-bit `command:` value is serialised into the
  ...
  The four Hayward / Jandy formats as explicit `command_format:` blocks (each shows only this
  block; combine with `framing`, `crc`, and `tx.gate` from the table above to form a valid hub):
...
| [swilson/aqualogic](https://github.com/swilson/aqualogic) | first byte after frame_type | `payload[2]` (2-byte) |
| [aquaweb](https://github.com/earlephilhower/aquaweb) | mixed; see source per field | translate per-field |
| Raw bus capture (Wireshark, dump_frames log) | `DLE` itself | DLE+STX stripped before `payload[0]` |
